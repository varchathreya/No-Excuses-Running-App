...
**How to apply:** Keep Clerk for app identity only. Authorize Calendar with the app-owned Google OAuth client and an HTTPS server callback, store encrypted refresh tokens per Clerk user, and request only `calendar.readonly`. Installed Android builds should return from the server callback through the app-owned scheme; Expo Go can only use a browser plus app-resume refresh. Only show Booked/time when the event is visible in that user's connected calendar.
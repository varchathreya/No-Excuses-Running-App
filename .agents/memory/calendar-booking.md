...
**How to apply:** Keep the per-session Book action as a mobile-safe Google Calendar template link. Only show Booked/time when the event is visible in the explicitly connected backend calendar, and label that account scope truthfully. Refresh on app resume plus a short polling interval.
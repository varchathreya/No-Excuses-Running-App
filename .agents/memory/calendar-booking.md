...
**How to apply:** Keep the per-session Book action as a mobile-safe Google Calendar template link. Authenticate app users with Clerk, request Calendar scope by reauthorizing their Google external account, and resolve the provider token on the server from the authenticated Clerk user. Only show Booked/time when the event is visible in that user's connected calendar. Refresh on app resume plus a short polling interval.
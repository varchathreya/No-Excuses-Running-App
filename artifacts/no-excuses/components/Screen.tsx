...
export function Button({ label, onPress, secondary = false, icon }: { label: string; onPress: () => void; secondary?: boolean; icon?: keyof typeof Feather.glyphMap }) {
  const colors = useColors();
  return <Pressable testID={label} onPress={onPress} style={({ pressed }) => [styles.button, { backgroundColor: secondary ? colors.secondary : colors.primary, opacity: pressed ? 0.78 : 1 }]}>{icon && <Feather name={icon} size={16} color={secondary ? colors.foreground : colors.primaryForeground} />}<Text style={[styles.buttonText, { color: secondary ? colors.foreground : colors.primaryForeground }]}>{label}</Text></Pressable>;
}
